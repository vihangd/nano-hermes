...
| **External skill dirs** | off (empty list) | Read-only directories of shared skills, discovered on each index refresh and searchable alongside workspace/builtin skills. Content is **injection-scanned on the read path** with the same scanner that gates `propose_skill` writes — an external dir is a shared folder, synced drive or cloned repo, i.e. the supply-chain position where hostile content arrives without anyone writing it through the agent. A skill whose body or description trips the scanner is dropped whole and logged, never redacted. Verdicts are memoised on `(mtime, size)` so the scan does not re-run on the foreground `skill_search` path | `skills.external_dirs` (list of paths; `~` and `${VAR}` expanded) |
...
| **Dynamic Cheatsheet** | off | After each completed session, one LLM call extracts a single transferable lesson **plus a one-line applicability condition** (`applies_when`). At iteration 0 the top-k lessons whose *condition* best matches the current task (KNN by embedding) are prepended. State-gating the match on the condition (not the lesson text) sharpens relevance at no extra per-turn cost | `skill_stats.cheatsheet_enabled`, `cheatsheet_top_k` (3) |
| **ExpeL contrastive insights** | off | After each session, finds the most similar past session with the opposite outcome (success ↔ fail/partial). One LLM call extracts a contrastive insight **and its applicability condition**, embedded into the lesson store so it's KNN-retrievable and condition-gated alongside cheatsheet lessons at iteration 0 | `expel_enabled` (top-level), `expel_similarity_threshold` (0.75) |
